...
function TipoDocumentoLista() {
    const [tipoDocumentoLista, setTipoDocumentoLista] = useState({ data: [] });
    ...
    const [getRoutes, setRoutes] = useState({
        ...
        crear_registro: "/TipoDocumento/Add",
        consultar_registros: "/TipoDocumento",
        muestra_atras: false,
    ...
    const handleCallback = (childData) => {
        ...
        cargarTiposDocumentos(childData);
    }

    const columns = [
        {
            name: 'NOMBRE',
            selector: tipoDoc => tipoDoc.attributes.nombre,
            sortable: true,
            width: "80%"
        },
        {
            name: 'ESTADO',
            selector: tipoDoc => (tipoDoc.attributes.estado == "1" ? 'ACTIVO' : 'INACTIVO'),
            sortable: true,
            width: "10%"
        },
        {
    ...
    useEffect(() => {
        async function fetchData() {
            cargarTiposDocumentos(1);
        }
        fetchData();
    }, []);

    const cargarTiposDocumentos = (estado) => {
        window.showSpinner(true);
        GenericApi.getAllGeneric('getTipoDocumento/'+estado).then(
            datos => {
                if (!datos.error) {
                    setTipoDocumentoLista(datos);
                    window.showSpinner(false);
                }else{
                    window.showSpinner(false);
                    setModalState({ title: "LISTA DE TIPOS DE DOCUMENTOS :: ERROR", message: datos.error.toString(), show: true, alert: global.Constants.TIPO_ALERTA.ERROR, button: true });
                }
        ...
        <div>
                    ...
                    <div className="w2d_block let">
                        <nav aria-label="breadcrumb">
                            <ol className="breadcrumb breadcrumb-alt push">
                                ...
                                <li className="breadcrumb-item"> <small>Tipos de documento de identificación</small></li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div className="block block-themed">
                <div className="block-header">
                    <h3 className="block-title">ADMINISTRACIÓN :: TIPOS DE DOCUMENTOS {getEstadoLista == global.Constants.ESTADOS.INACTIVO ? "INACTIVOS" : "ACTIVOS"}</h3>
                </div>
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            <DataTable className="table table-bordered table-striped table-vcenter js-dataTable-full"
                                columns={columns}
                                data={tipoDocumentoLista.data.filter((suggestion) => {
                                    if (getSeach === "") {
                                ...
                                paginationTotalRows={tipoDocumentoLista.data.length}
...
export default TipoDocumentoLista;