...
function LocalidadLista() {
    const [localidadListaLista, setLocalidadListaLista] = useState({ data: [] });
    ...
    const [getRoutes, setRoutes] = useState({
        ...
        crear_registro: "/Localidad/Add",
        consultar_registros: "/Localidad",
        muestra_atras: false,
    ...
    const handleCallback = (childData) => {
        ...
        cargarLocalidades(childData);
    }

    const columns = [
        {
            name: 'NOMBRE',
            selector: localidad => localidad.attributes.nombre,
            sortable: true,
            width: "80%"
        },
        {
            name: 'ESTADO',
            selector: localidad => (localidad.attributes.estado == "1" ? 'ACTIVO' : 'INACTIVO'),
            sortable: true,
            width: "10%"
        },
        {
    ...
    const cargarLocalidades = (estado) => {
        window.showSpinner(true);
        GenericApi.getAllGeneric('getMasLocalidad/'+estado).then(
            datos => {
                if (!datos.error) {
                    setLocalidadListaLista(datos);
                    window.showSpinner(false);
                }
                else {
                    window.showSpinner(false);
                    setModalState({ title: "LISTA DE LOCALIDADES :: ERROR", message: datos.error.toString(), show: true, alert: global.Constants.TIPO_ALERTA.ERROR, button: true });
                }
        ...
        <div>
                ...
                <div className="w2d_block let">
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb breadcrumb-alt push">
                            ...
                            <li className="breadcrumb-item"> <small>Localidades</small></li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div className="block block-themed">
                <div className="block-header">
                    <h3 className="block-title">ADMINISTRACIÓN :: LOCALIDADES {getEstadoLista == global.Constants.ESTADOS.INACTIVO ? "INACTIVAS" : "ACTIVAS"}</h3>
                </div>
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            <DataTable className="table table-bordered table-striped table-vcenter js-dataTable-full"
                                ...
                                data={localidadListaLista.data.filter((suggestion) => {
                                    if (getSeach === "") {
                                ...
                                paginationTotalRows={localidadListaLista.data.length}
