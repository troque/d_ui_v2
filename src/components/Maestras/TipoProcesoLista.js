...
function TipoProcesoLista() {

    const [getTipoProcesoLista, setTipoProcesoLista] = useState({ data: [] });
    ...
    const [getRoutes, setRoutes] = useState({
        crear_registro: "/TipoProceso/Add",
        consultar_registros: "/TipoProceso/Lista",
        muestra_atras: false,
    ...
    const columns = [
        {
            name: 'NOMBRE',
            selector: tipo_proceso => tipo_proceso.attributes.nombre,
            sortable: true,
        },
        {
            name: 'ESTADO',
            selector: tipo_proceso => (tipo_proceso.attributes.estado == "1" ? 'ACTIVO' : 'INACTIVO'),
            sortable: true,
        },
        {
            name: 'ACCIONES',
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            cargarCiudad(1, paganationPerPages);
        }
        fetchData();
    }, []);


    const cargarCiudad = (page, perPage) => {

        GenericApi.getAllGeneric('mas-tipo-proceso').then(
            datos => {
                if (!datos.error) {
                    setTipoProcesoLista(datos);
        ...
        <div>
            <div className="w2d_block let">
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb breadcrumb-alt push">
                        ...
                        <li className="breadcrumb-item"> <small>Tipos del proceso</small></li>
                    </ol>
                </nav>
            </div>
            {<InfoErrorApi />}
            ...
            <div className="block block-themed">
                <div className="block-header">
                    <h3 className="block-title">ADMINISTRACIÓN :: TIPOS DE PROCESOS REGISTRADOS {getEstadoLista == global.Constants.ESTADOS.INACTIVO ? "INACTIVOS" : "ACTIVOS"}</h3>
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            <DataTable className="table table-bordered table-striped table-vcenter js-dataTable-full"
                                columns={columns}
                                data={getTipoProcesoLista.data.filter((suggestion) => {
                                    if (getSeach === "") {
                                ...
                                paginationTotalRows={getTipoProcesoLista.data.length}
...
export default TipoProcesoLista;