...
function CargosLista() {
    ...
    const [getCargosLista, setCargosLista] = useState({ data: [] });
    ...
    const [getRoutes, setRoutes] = useState({
        crear_registro: "/Cargos/Add",
        consultar_registros: "/Cargos",
        muestra_atras: false,
    ...
    // Metodo encargado de setear los valores inactivos
    const handleCallback = (childData) => {
        setEstadoLista(childData);
        cargarCargos(childData);
    }
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            // Se llama el metodo encargado de cargar la informacion
            cargarCargos(1);
        }
        fetchData();
    }, []);

    const cargarCargos = (estado) => {
        window.showSpinner(true);
        GenericApi.getAllGeneric('getCargos/'+estado).then(
            datos => {
                if (!datos.error) {
                    setCargosLista(datos);
                    window.showSpinner(false);
                }
                else{
                    window.showSpinner(false);
                    setModalState({ title: "LISTA DE NIVEL JERÁRQUICO :: ERROR", message: datos.error.toString(), show: true, alert: global.Constants.TIPO_ALERTA.ERROR, button: true });
                }
            }
        )
    }
        ...
        <div>
            ...
            <div className="w2d_block let">
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb breadcrumb-alt push">
                        ...
                        <li className="breadcrumb-item"> <small>Nivel Jerárquico</small></li>
                    </ol>
                </nav>
            </div>
            ...
            <div className="block block-themed">
                <div className="block-header">
                    <h3 className="block-title">ADMINISTRACIÓN :: NIVELES JERÁRQUICOS {getEstadoLista == global.Constants.ESTADOS.INACTIVO ? "INACTIVOS" : "ACTIVOS"}</h3>
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            <DataTable className="table table-bordered table-striped table-vcenter js-dataTable-full"
                                columns={columns}
                                data={getCargosLista.data.filter((suggestion) => {
                                    if (getSeach === "") {
                                ...
                                paginationTotalRows={getCargosLista.data.length}
